...
const ArticleRuEditDialog: React.FC<ArticleRuEditDialogProps> = ({
  ...
  const loadImages = async () => {
    if (!articleRuId) return;
    try {
      const res = await articleRuService.getArticleRuImages(articleRuId);
      if (res.data) {
        setImages(res.data);
      }
    } catch (err) {
  ...
  const handleUploadByUrl = async () => {
    if (!articleRuId || !uploadUrl || !uploadFilename) return;
    try {
      setLocalSaving(true);
      await articleRuService.uploadArticleRuImageByUrl({
        articleRuId,
        articleRuTitle: formData.title,
  ...
  const handleUploadByFile = async () => {
    if (!articleRuId || !uploadFile || !uploadFileFilename) return;
    try {
      setLocalSaving(true);
      await articleRuService.uploadArticleRuImageByFile({
        articleRuId,
        articleRuTitle: formData.title,
        file: uploadFile,
  ...
  const handleConfirmDeleteImage = async () => {
    ...
    try {
      setLocalSaving(true);
      await articleRuService.deleteArticleRuImage(imageToDelete);
      await loadImages();
    } catch (err) {
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      console.error('[ArticleRuEditDialog] Error loading tags:', err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableLangOptions = useMemo(() => {
    try {
      ...
      console.error('[ArticleRuEditDialog] Error loading lang options:', err);
  ...
  return (
    ...
    >
      ...
      <DialogTitle>{t('articleRus.edit')}</DialogTitle>
      <DialogContent>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          ...
          <Box sx={{ border: '1px solid #ddd', p: 2, borderRadius: 1 }}>
            <Typography variant="subtitle1" gutterBottom>
              {t('articleRus.form.coverImageFile')}
            </Typography>
            ...
            <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 2 }}>
              ...
              <Typography variant="body2" sx={{ ml: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 200 }}>
                {formData.coverImageFile
                  ...
                  : formData.coverImageFilename || t('articleRus.messages.noFileSelected', 'No file selected')}
              </Typography>
            </Box>
          ...
          <FormControl fullWidth>
            <FormLabel sx={{ mb: 1 }}>{t('articleRus.form.term')}</FormLabel>
            <Select
          ...
          <FormControl fullWidth>
            <FormLabel sx={{ mb: 1 }}>{t('articleRus.form.week')}</FormLabel>
            <Select
      ...
      >
        <DialogTitle>{t('articleRus.actions.delete')}</DialogTitle>
        <DialogContent>
          <Typography>{t('articleRus.messages.confirmDeleteImage')}</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDeleteConfirmOpen(false)}>{t('articleRus.actions.cancel')}</Button>
...
export default ArticleRuEditDialog;
