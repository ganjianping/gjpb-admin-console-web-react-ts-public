...
import type { TrueFalseQuestionRuSearchFormData } from '../types/trueFalseQuestionRu.types';
import { 
  TRUE_FALSE_QUESTION_TAG_SETTING_KEY, 
  LANGUAGE_OPTIONS, 
  DIFFICULTY_LEVEL_OPTIONS,
  TRUE_FALSE_QUESTION_LANG_SETTING_KEY,
  TRUE_FALSE_QUESTION_DIFFICULTY_LEVEL_SETTING_KEY
} from '../constants';

interface TrueFalseQuestionRuSearchPanelProps {
  searchFormData: TrueFalseQuestionRuSearchFormData;
  loading?: boolean;
  onFormChange: (field: keyof TrueFalseQuestionRuSearchFormData, value: any) => void;
  onSearch: () => void;
  onClear: () => void;
}

const TrueFalseQuestionRuSearchPanel: React.FC<TrueFalseQuestionRuSearchPanelProps> = ({
  searchFormData,
  loading,
  onFormChange,
  onSearch,
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      const tagSetting = appSettings.find((s) => s.name === TRUE_FALSE_QUESTION_TAG_SETTING_KEY && s.lang === currentLang);
      if (!tagSetting) return [] as string[];
      return tagSetting.value.split(',').map((v) => v.trim()).filter(Boolean);
    } catch (err) {
      console.error('[TrueFalseQuestionRuSearchPanel] Error loading tags:', err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableLangOptions = useMemo(() => {
    try {
      ...
      const currentLang = i18n.language.toUpperCase().startsWith('ZH') ? 'ZH' : 'EN';
      const langSetting =
        appSettings.find((s) => s.name === TRUE_FALSE_QUESTION_LANG_SETTING_KEY && s.lang === currentLang) ||
        appSettings.find((s) => s.name === TRUE_FALSE_QUESTION_LANG_SETTING_KEY);
      ...
      console.error('[TrueFalseQuestionRuSearchPanel] Error loading lang options:', err);
  ...
  const availableDifficultyOptions = useMemo(() => {
    try {
      ...
      const difficultySetting =
        appSettings.find((s) => s.name === TRUE_FALSE_QUESTION_DIFFICULTY_LEVEL_SETTING_KEY && s.lang === currentLang) ||
        appSettings.find((s) => s.name === TRUE_FALSE_QUESTION_DIFFICULTY_LEVEL_SETTING_KEY);
      ...
      console.error('[TrueFalseQuestionRuSearchPanel] Error loading difficulty options:', err);
  ...
  return (
    ...
    >
      <CardContent sx={{ p: 3 }}>
        ...
        >
          <Search size={20} />
          {t('trueFalseQuestionRus.search')}
        </Typography>

        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: 'repeat(4, 1fr)' }, gap: 2, mb: 3 }}>
          <FormControl fullWidth>
            <FormLabel sx={{ mb: 1, fontSize: '0.875rem', fontWeight: 500 }}>
              {t('trueFalseQuestionRus.filters.searchByQuestion')}
            </FormLabel>
            <TextField
              size="small"
              placeholder={t('trueFalseQuestionRus.filters.searchByQuestion')}
          ...
          <FormControl fullWidth>
            <FormLabel sx={{ mb: 1, fontSize: '0.875rem', fontWeight: 500 }}>
              {t('trueFalseQuestionRus.form.lang')}
            </FormLabel>
            <Select
              size="small"
            ...
            >
              <MenuItem value="">{t('trueFalseQuestionRus.filters.all')}</MenuItem>
          ...
          <FormControl fullWidth>
            <FormLabel sx={{ mb: 1, fontSize: '0.875rem', fontWeight: 500 }}>
              {t('trueFalseQuestionRus.form.tags')}
            </FormLabel>
            <Select
              size="small"
          ...
          <FormControl fullWidth>
            <FormLabel sx={{ mb: 1, fontSize: '0.875rem', fontWeight: 500 }}>
              {t('trueFalseQuestionRus.form.difficultyLevel')}
            </FormLabel>
            <Select
              size="small"
          ...
          <FormControl fullWidth>
            <FormLabel sx={{ mb: 1, fontSize: '0.875rem', fontWeight: 500 }}>
              {t('trueFalseQuestionRus.form.status')}
            </FormLabel>
            <Select
              size="small"
            ...
            >
              <MenuItem value="">{t('trueFalseQuestionRus.filters.all')}</MenuItem>
              <MenuItem value="true">{t('trueFalseQuestionRus.status.active')}</MenuItem>
              <MenuItem value="false">{t('trueFalseQuestionRus.status.inactive')}</MenuItem>
            </Select>
          </FormControl>
        </Box>
        ...
        <Box sx={{ display: 'flex', gap: 1.5, justifyContent: 'flex-end' }}>
          ...
          >
            {t('trueFalseQuestionRus.clearFilters')}
          </Button>
          <Button
            variant="contained"
...
export default TrueFalseQuestionRuSearchPanel;
