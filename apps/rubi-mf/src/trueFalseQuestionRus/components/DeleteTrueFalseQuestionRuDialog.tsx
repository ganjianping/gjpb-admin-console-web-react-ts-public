...
interface DeleteTrueFalseQuestionRuDialogProps {
  open: boolean;
  trueFalseQuestionRu: TrueFalseQuestionRu | null;
  onClose: () => void;
  onConfirm: (trueFalseQuestionRu: TrueFalseQuestionRu) => Promise<void>;
}

const DeleteTrueFalseQuestionRuDialog = ({ open, trueFalseQuestionRu, onClose, onConfirm }: DeleteTrueFalseQuestionRuDialogProps) => {
  const { t } = useTranslation();
  const [deleting, setDeleting] = useState(false);

  const handleConfirm = async () => {
    if (!trueFalseQuestionRu) return;
    setDeleting(true);
    try {
      await onConfirm(trueFalseQuestionRu);
      onClose();
    } finally {
      setDeleting(false);
    }
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ pb: 1, display: 'flex', alignItems: 'center', gap: 1.5, borderBottom: '1px solid', borderColor: 'divider' }}>
        ...
        <Typography variant="h6" component="span">
          {t('trueFalseQuestionRus.delete')}
        </Typography>
      </DialogTitle>
      <DialogContent sx={{ pt: 3 }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Alert severity="warning">{t('trueFalseQuestionRus.messages.deleteConfirmation')}</Alert>
          {trueFalseQuestionRu && (
            <Box sx={{ p: 2, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 1.5 }}>
              <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 0.5 }}>
                {trueFalseQuestionRu.question}
              </Typography>
...
export default DeleteTrueFalseQuestionRuDialog;
