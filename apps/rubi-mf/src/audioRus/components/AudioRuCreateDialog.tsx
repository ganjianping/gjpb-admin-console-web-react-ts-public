...
interface AudioRuCreateDialogProps {
  open: boolean;
  onClose: () => void;
  formData: AudioRuFormData;
  onFormChange: (field: keyof AudioRuFormData, value: any) => void;
  loading?: boolean;
...
const AudioRuCreateDialog = ({
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      const audioRuTagsSetting = appSettings.find((s) => s.name === 'audio_ru_tags' && s.lang === currentLang);
      if (!audioRuTagsSetting) return [] as string[];
      return audioRuTagsSetting.value.split(',').map((v) => v.trim()).filter(Boolean);
    } catch (err) {
      console.error('[AudioRuCreateDialog] Error loading tags:', err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableLangOptions = useMemo(() => {
    try {
      ...
      console.error('[AudioRuCreateDialog] Error loading lang options:', err);
  ...
  const handleFileChange = (field: keyof AudioRuFormData, e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = async () => {
    ...
    setLocalSaving(true);
    try {
      if (!formData.file) throw new Error('No audioRu file selected');
      ...
      if (onCreated) {
        try {
          ...
          console.error('[AudioRuCreateDialog] onCreated callback failed', err);
        }
      }
      onClose();
    } catch (err: any) {
  ...
  return (
    ...
    >
      ...
      <DialogTitle sx={{ pt: loading ? 3 : 2 }}>
        <Typography variant="h6" component="div">{t('audioRus.create') || 'Create AudioRu'}</Typography>
      </DialogTitle>
      <DialogContent sx={{ pt: 2 }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
          <TextField label={t('audioRus.form.name') || 'Name'} value={formData.name} onChange={(e) => onFormChange('name', e.target.value)} fullWidth error={!!getFieldError('name')} helperText={getFieldError('name')} />
          <TextField label={t('audioRus.form.artist') || 'Artist'} value={(formData as any).artist || ''} onChange={(e) => onFormChange('artist' as any, e.target.value)} fullWidth />
          <Box>
            <Typography variant="subtitle2">{t('audioRus.form.audioRuFile') || 'AudioRu File'}</Typography>
            <input type="file" accept="audioRu/*" onChange={(e) => handleFileChange('file', e)} />
          </Box>
          <TextField label={t('audioRus.form.filename') || 'Filename'} value={formData.filename || ''} onChange={(e) => onFormChange('filename', e.target.value)} fullWidth />
          <Box>
            <Typography variant="subtitle2">{t('audioRus.form.coverImageFile') || 'Cover Image File'}</Typography>
            <input type="file" accept="image/*" onChange={(e) => handleFileChange('coverImageFile', e)} />
          </Box>
          <TextField label={t('audioRus.form.coverImageFilename') || 'Cover Image Filename'} value={formData.coverImageFilename || ''} onChange={(e) => onFormChange('coverImageFilename' as any, e.target.value)} fullWidth />
          ...
          <FormControlLabel control={<Switch checked={formData.isActive} onChange={(e) => onFormChange('isActive', e.target.checked)} />} label={t('audioRus.form.isActive') || 'Active'} />

          <TextField label={t('audioRus.form.sourceName') || 'Source Name'} value={(formData as any).sourceName || ''} onChange={(e) => onFormChange('sourceName' as any, e.target.value)} fullWidth />
          <TextField label={t('audioRus.form.originalUrl') || 'Original URL'} value={(formData as any).originalUrl || ''} onChange={(e) => onFormChange('originalUrl' as any, e.target.value)} fullWidth />
          <Box>
            <Typography variant="subtitle2">{t('audioRus.form.description') || 'Description'}</Typography>
            <TextareaAutosize
              minRows={3}
              style={{ width: '100%', padding: '8.5px 14px', borderRadius: 4, border: '1px solid rgba(0,0,0,0.23)', fontFamily: 'inherit' }}
              value={formData.description || ''}
              onChange={(e) => onFormChange('description', e.target.value)}
              aria-label={t('audioRus.form.description') || 'Description'}
            />
      ...
      <DialogActions>
        <Button onClick={() => { if (onReset) { onReset(); } onClose(); }} disabled={loading || localSaving}>{t('audioRus.actions.cancel') || 'Cancel'}</Button>
        <Button variant="contained" onClick={handleSubmit} disabled={loading || localSaving} startIcon={(loading || localSaving) ? <CircularProgress size={16} color="inherit" /> : undefined}>{t('audioRus.actions.save') || 'Save'}</Button>
      </DialogActions>
      <Backdrop sx={{ position: 'absolute', zIndex: (theme) => theme.zIndex.drawer + 1, backgroundColor: 'rgba(0,0,0,0.6)' }} open={loading || localSaving}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, p: 4, borderRadius: 2, backgroundColor: 'background.paper' }}>
          ...
          <Typography>{t('audioRus.messages.pleaseWait') || 'Please wait...'}</Typography>
        </Box>
      </Backdrop>
    </Dialog>
  );
...
export default AudioRuCreateDialog;
