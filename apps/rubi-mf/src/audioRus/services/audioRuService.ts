...
import type { AudioRu, AudioRuPaginatedResponse } from "../types/audioRu.types";

export interface AudioRuQueryParams {
  page?: number;
  size?: number;
  sort?: string;
  direction?: "asc" | "desc";
...
export interface CreateAudioRuByUploadRequest {
  file: File;
  name: string;
  filename: string;
  coverImageFilename: string;
...
class AudioRuService {
  private readonly getUrl = "/v1/audio-rus";
  private readonly crudUrl = "/v1/audio-rus";

  async getAudioRus(params?: AudioRuQueryParams): Promise<ApiResponse<AudioRuPaginatedResponse>> {
    return apiClient.get(this.getUrl, { params });
  }


  async createAudioRu(data: CreateAudioRequest): Promise<ApiResponse<AudioRu>> {
    return apiClient.post(this.crudUrl, data);
  }

  async createAudioRuByUpload(data: CreateAudioRuByUploadRequest): Promise<ApiResponse<AudioRu>> {
  ...
  async updateAudioRu(id: string, data: UpdateAudioRequest): Promise<ApiResponse<AudioRu>> {
  ...
  async updateAudioRuWithFiles(id: string, data: UpdateAudioRequest & { file?: File | null; coverImageFile?: File | null }): Promise<ApiResponse<AudioRu>> {
    const formData = new FormData();
    // Do not allow changing the primary audioRu file via Edit — only allow cover image updates here.
    if ((data as any).coverImageFile) {
  ...
  async deleteAudioRu(id: string): Promise<ApiResponse<void>> {
    return apiClient.delete(`${this.crudUrl}/${id}`);
  }
}

export const audioRuService = new AudioRuService();
