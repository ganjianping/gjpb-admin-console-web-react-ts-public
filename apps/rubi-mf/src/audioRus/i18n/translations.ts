import i18n from '../../../../shared-lib/src/i18n/i18n';

const enTranslations = {
  audioRus: {
    title: 'Audios',
    subtitle: 'Manage audios and related information',
    description: 'Maintain the audio library and metadata',
    pageTitle: 'Audios Management',
    create: 'Create Audio',
    edit: 'Edit Audio',
    delete: 'Delete Audio',
    view: 'View Audio',
    search: 'Search Audios',
    clearFilters: 'Clear Filters',
    noAudioRusFound: 'No audios found',
    columns: {
      cover: 'Cover',
      name: 'Name',
      artist: 'Artist',
      extension: 'Extension',
      lang: 'Language',
      tags: 'Tags',
      displayOrder: 'Order',
      isActive: 'Status',
      updatedAt: 'Updated At',
    },
    form: {
      name: 'Audio Name',
      subtitle: 'Subtitle',
      description: 'Description',
      audioRuFile: 'Audio File',
      coverImageFile: 'Cover Image File',
      coverImageFilename: 'Cover Image Filename',
      sourceName: 'Source Name',
      originalUrl: 'Original URL',
      artist: 'Artist',
      filename: 'Filename',
      thumbnailFilename: 'Thumbnail Filename',
      extension: 'Extension',
      mimeType: 'MIME Type',
      sizeBytes: 'Size (bytes)',
      width: 'Width',
      height: 'Height',
      altText: 'Alt Text',
      tags: 'Tags',
      lang: 'Language',
      displayOrder: 'Display Order',
      isActive: 'Active Status',
      status: 'Status',
      uploadMethod: 'Upload Method',
      byUrl: 'By URL',
      uploadFile: 'Upload File',
      chooseFile: 'Choose File',
    },
    filters: {
      searchByName: 'Search by name',
      all: 'All',
    },
    actions: {
      view: 'View',
      edit: 'Edit',
      delete: 'Delete',
      create: 'Create',
      cancel: 'Cancel',
      save: 'Save',
      copyFilename: 'Copy Filename',
  copyThumbnail: 'Copy Cover Image',
      more: 'More...',
      less: 'Less',
    },
    messages: {
      filenameCopied: 'Filename copied to clipboard',
      thumbnailFilenameCopied: 'Thumbnail filename copied to clipboard',
      savingAudioRu: 'Saving audioRu...',
      uploadingAudioRu: 'Uploading audioRu...',
      pleaseWait: 'Please wait...',
      deleteWarning: 'This action cannot be undone.',
      deleteConfirm: 'Are you sure you want to delete this audioRu?',
      createSuccess: 'AudioRu created successfully',
      updateSuccess: 'AudioRu updated successfully',
      deleteSuccess: 'AudioRu deleted successfully',
    },
    errors: {
      loadFailed: 'Failed to load audioRus',
      fileRequired: 'Please select a file to upload',
    },
    languages: {
      EN: 'English',
      ZH: 'Chinese',
    },
    status: {
      active: 'Active',
      inactive: 'Inactive',
    },
    viewDialog: {
      details: 'Details',
      id: 'ID',
      filename: 'Filename',
      extension: 'Extension',
      language: 'Language',
      displayOrder: 'Display Order',
      originalUrl: 'Original URL',
      artist: 'Artist',
      tags: 'Tags',
      metadata: 'Metadata',
      createdAt: 'Created At',
      updatedAt: 'Updated At',
      createdBy: 'Created By',
      updatedBy: 'Updated By',
    },
  },
};

const zhTranslations = {
  audioRus: {
    title: '音频管理',
    subtitle: '管理音频及相关信息',
    description: '维护音频库及其元数据',
    pageTitle: '音频管理',
    create: '创建音频',
    edit: '编辑音频',
    delete: '删除音频',
    view: '查看音频',
    search: '搜索音频',
    clearFilters: '清除筛选',
    noAudioRusFound: '未找到音频',
    columns: {
      cover: '封面',
      name: '名称',
      artist: '艺术家',
      extension: '扩展名',
      lang: '语言',
      tags: '标签',
      displayOrder: '排序',
      isActive: '状态',
      updatedAt: '更新时间',
    },
    form: {
      name: '音频名称',
      subtitle: '字幕',
      coverImageFilename: '封面图片文件名',
      sourceName: '来源名称',
      originalUrl: '原始链接',
      artist: '艺术家',
      filename: '文件名',
      thumbnailFilename: '缩略图文件名',
      extension: '扩展名',
      mimeType: 'MIME 类型',
      sizeBytes: '文件大小（字节）',
      width: '宽度',
      height: '高度',
      altText: '替代文本',
      tags: '标签',
      lang: '语言',
      displayOrder: '显示顺序',
      isActive: '启用状态',
      status: '状态',
      uploadMethod: '上传方式',
      byUrl: '通过链接',
      uploadFile: '上传文件',
      chooseFile: '选择文件',
    },
    filters: {
      searchByName: '按名称搜索',
      all: '全部',
    },
    actions: {
      view: '查看',
      edit: '编辑',
      delete: '删除',
      create: '创建',
      cancel: '取消',
      save: '保存',
      copyFilename: '复制文件名',
  copyThumbnail: '复制封面图片文件名',
      more: '更多',
      less: '收起',
    },
    messages: {
      filenameCopied: '文件名已复制到剪贴板',
      thumbnailFilenameCopied: '缩略图文件名已复制到剪贴板',
      savingAudioRu: '正在保存音频...',
      uploadingAudioRu: '正在上传音频...',
      pleaseWait: '请稍候...',
      deleteWarning: '此操作无法撤销。',
      deleteConfirm: '确定要删除此音频吗？',
      createSuccess: '音频创建成功',
      updateSuccess: '音频更新成功',
      deleteSuccess: '音频删除成功',
    },
    errors: {
      loadFailed: '加载音频失败',
      fileRequired: '请选择要上传的文件',
    },
    languages: {
      EN: '英语',
      ZH: '中文',
    },
    status: {
      active: '启用',
      inactive: '禁用',
    },
    viewDialog: {
      details: '详细信息',
      id: 'ID',
      filename: '文件名',
      extension: '扩展名',
      language: '语言',
      displayOrder: '显示顺序',
      originalUrl: '原始链接',
      artist: '艺术家',
      tags: '标签',
      metadata: '元数据',
      createdAt: '创建时间',
      updatedAt: '更新时间',
      createdBy: '创建人',
      updatedBy: '更新人',
    },
  },
};


i18n.addResourceBundle('en', 'translation', enTranslations, true, true);
i18n.addResourceBundle('zh', 'translation', zhTranslations, true, true);
