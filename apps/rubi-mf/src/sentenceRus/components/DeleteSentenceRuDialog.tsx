...
interface DeleteSentenceRuDialogProps {
  open: boolean;
  sentenceRu: SentenceRu | null;
  onClose: () => void;
  onConfirm: (sentenceRu: SentenceRu) => Promise<void>;
}

const DeleteSentenceRuDialog = ({ open, sentenceRu, onClose, onConfirm }: DeleteSentenceRuDialogProps) => {
  const { t } = useTranslation();
  const [deleting, setDeleting] = useState(false);

  const handleConfirm = async () => {
    if (!sentenceRu) return;
    setDeleting(true);
    try {
      await onConfirm(sentenceRu);
      onClose();
    } finally {
      setDeleting(false);
    }
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ pb: 1, display: 'flex', alignItems: 'center', gap: 1.5, borderBottom: '1px solid', borderColor: 'divider' }}>
        ...
        <Typography variant="h6" component="span">
          {t('sentenceRus.delete')}
        </Typography>
      </DialogTitle>
      <DialogContent sx={{ pt: 3 }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Alert severity="warning">{t('sentenceRus.messages.deleteConfirmation')}</Alert>
          {sentenceRu && (
...
export default DeleteSentenceRuDialog;
