...
interface VideoRuSearchPanelProps {
  searchFormData: VideoRuSearchFormData;
  loading?: boolean;
  onFormChange: (field: keyof VideoRuSearchFormData, value: any) => void;
  onSearch: () => void;
  onClear: () => void;
}

const VideoRuSearchPanel: React.FC<VideoRuSearchPanelProps> = ({ searchFormData, loading, onFormChange, onSearch, onClear }) => {
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      console.error('[VideoRuSearchPanel] Error loading tags:', err);
      return [] as string[];
    }
  }, [i18n.language]);
    ...
    <Card elevation={0} sx={{
      ...
      <CardContent sx={{ position: 'relative', zIndex: 2, p: 3 }}>
        ...
        <Box sx={{
          ...
          <Box>
            <FormLabel sx={{ fontWeight: 500, color: 'text.primary', mb: 1, display: 'block' }}>{t('videoRus.form.name') || t('videoRus.filters.searchByName')}</FormLabel>
            <TextField fullWidth size="small" value={searchFormData.name || ''} onChange={(e) => onFormChange('name', e.target.value)} placeholder={t('videoRus.filters.searchByName')} disabled={loading} sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }} />
          </Box>

          <Box>
            <FormLabel sx={{ fontWeight: 500, color: 'text.primary', mb: 1, display: 'block' }}>{t('videoRus.form.lang')}</FormLabel>
            <FormControl fullWidth size="small">
              <Select value={searchFormData.lang || ''} onChange={(e) => onFormChange('lang', e.target.value)} disabled={loading} displayEmpty sx={{ borderRadius: 2 }}>
                <MenuItem value="">{t('videoRus.filters.all')}</MenuItem>
                {LANGUAGE_OPTIONS.map((option) => (
                  <MenuItem key={option.value} value={option.value}>{t(`videoRus.languages.${option.value}`)}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </Box>
          ...
          <Box>
            <FormLabel sx={{ fontWeight: 500, color: 'text.primary', mb: 1, display: 'block' }}>{t('videoRus.form.tags')}</FormLabel>
            <FormControl fullWidth size="small">
              <Select<string[]>
                multiple
                ...
                renderValue={(selected) => {
                  if (selected.length === 0) return (<Typography variant="body2" color="text.disabled">{t('videoRus.filters.all')}</Typography>);
          ...
          <Box>
            <FormLabel sx={{ fontWeight: 500, color: 'text.primary', mb: 1, display: 'block' }}>{t('videoRus.form.status') || 'Status'}</FormLabel>
            <FormControl fullWidth size="small">
              <Select value={searchFormData.isActive || ''} onChange={(e) => onFormChange('isActive', e.target.value)} disabled={loading} displayEmpty sx={{ borderRadius: 2 }}>
                <MenuItem value="">{t('videoRus.filters.all')}</MenuItem>
                <MenuItem value="true">{t('videoRus.status.active')}</MenuItem>
                <MenuItem value="false">{t('videoRus.status.inactive')}</MenuItem>
              </Select>
            </FormControl>
          </Box>
        </Box>
...
export default VideoRuSearchPanel;
