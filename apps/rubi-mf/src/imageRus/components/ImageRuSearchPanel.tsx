...
import type { ImageRuSearchFormData } from '../types/imageRu.types';
import { LANGUAGE_OPTIONS } from '../constants';

interface ImageRuSearchPanelProps {
  searchFormData: ImageRuSearchFormData;
  loading: boolean;
  onFormChange: (field: keyof ImageRuSearchFormData, value: any) => void;
  onSearch: () => void;
  onClear: () => void;
}

const ImageRuSearchPanel: React.FC<ImageRuSearchPanelProps> = ({
  searchFormData,
  loading,
  onFormChange,
  onSearch,
  ...
  // Get imageRu tags from local storage filtered by current language
  const availableTags = useMemo(() => {
    try {
      ...
      console.error('[ImageRuSearchPanel] Error loading tags:', error);
      return [];
    }
  }, [i18n.language]);
        ...
        <Box sx={{
          ...
          <Box>
            ...
            <TextField
              ...
              placeholder={t('imageRus.filters.searchByName')}
              disabled={loading}
              sx={{
          ...
          <Box>
            ...
            <FormControl fullWidth size="small">
              ...
              >
                <MenuItem value="">{t('imageRus.filters.all')}</MenuItem>
                {LANGUAGE_OPTIONS.map((option) => (
                  <MenuItem key={option.value} value={option.value}>
                    {t(`imageRus.languages.${option.value}`)}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
          ...
          <Box>
            ...
            <FormControl fullWidth size="small">
              ...
              >
                <MenuItem value="">{t('imageRus.filters.all')}</MenuItem>
                <MenuItem value="true">{t('imageRus.status.active')}</MenuItem>
                <MenuItem value="false">{t('imageRus.status.inactive')}</MenuItem>
              </Select>
            </FormControl>
          </Box>
        </Box>
...
export default ImageRuSearchPanel;
