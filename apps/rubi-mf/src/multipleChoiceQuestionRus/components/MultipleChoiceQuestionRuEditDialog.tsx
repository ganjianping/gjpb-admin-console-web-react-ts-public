...
> = ({ open, multipleChoiceQuestionRu, onClose, onConfirm }) => {
  ...
  const availableGrammarChapters = useMemo(() => {
    try {
      ...
      console.error("[MultipleChoiceQuestionRuEditDialog] Error loading grammar chapters:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableScienceChapters = useMemo(() => {
    try {
      ...
      console.error("[MultipleChoiceQuestionRuEditDialog] Error loading science chapters:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  useEffect(() => {
    if (open && multipleChoiceQuestionRu) {
      setFormData({
        ...
        grammarChapter: multipleChoiceQuestionRu.grammarChapter || '',
        scienceChapter: multipleChoiceQuestionRu.scienceChapter || '',
