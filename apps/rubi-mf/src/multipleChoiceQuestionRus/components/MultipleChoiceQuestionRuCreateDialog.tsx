...
import type { MultipleChoiceQuestionRuFormData, QuestionImageRu } from "../types/multipleChoiceQuestionRu.types";
...
interface MultipleChoiceQuestionRuCreateDialogProps {
  open: boolean;
  onClose: () => void;
...
const MultipleChoiceQuestionRuCreateDialog: React.FC<
  MultipleChoiceQuestionRuCreateDialogProps
> = ({ open, onClose, onConfirm, createdQuestionId }) => {
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      console.error("[MultipleChoiceQuestionRuCreateDialog] Error loading tags:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableDifficultyLevels = useMemo(() => {
    try {
      ...
      console.error("[MultipleChoiceQuestionRuCreateDialog] Error loading difficulty levels:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableGrammarChapters = useMemo(() => {
    try {
      ...
      console.error("[MultipleChoiceQuestionRuCreateDialog] Error loading grammar chapters:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableScienceChapters = useMemo(() => {
    try {
      ...
      console.error("[MultipleChoiceQuestionRuCreateDialog] Error loading science chapters:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  useEffect(() => {
    if (open) {
      ...
      setFormData({ ...getEmptyMultipleChoiceQuestionRuFormData(), lang: defaultLang });
      setError(null);
  ...
  }, [open, i18n.language, createdQuestionId]);

  const loadImages = async () => {
    if (!createdQuestionId) return;
    try {
      const res = await multipleChoiceQuestionRuService.getQuestionImages(createdQuestionId);
      if (res.data) {
        setImages(res.data);
      }
    } catch (err) {
  ...
  const handleUploadByUrl = async () => {
    if (!createdQuestionId || !uploadUrl || !uploadFilename) return;
    try {
      setLocalSaving(true);
  ...
  const handleUploadByFile = async () => {
    if (!createdQuestionId || !uploadFile || !uploadFileFilename) return;
    try {
      setLocalSaving(true);
    ...
    >
      ...
      >
        ...
        <Typography variant="h6" component="span">
          {t("multipleChoiceQuestionRus.createDialog.title")}
        </Typography>
      </DialogTitle>
...
export default MultipleChoiceQuestionRuCreateDialog;
