...
interface ExpressionRuCreateDialogProps {
  open: boolean;
...
const ExpressionRuCreateDialog: React.FC<ExpressionRuCreateDialogProps> = ({
  open,
  formData,
  loading,
  onFormChange,
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      console.error('[ExpressionRuCreateDialog] Error loading tags:', err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableDifficultyLevels = useMemo(() => {
    try {
      ...
      console.error('[ExpressionRuCreateDialog] Error loading difficulty levels:', err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>{t('expressionRus.create')}</DialogTitle>
      <DialogContent>
...
export default ExpressionRuCreateDialog;
