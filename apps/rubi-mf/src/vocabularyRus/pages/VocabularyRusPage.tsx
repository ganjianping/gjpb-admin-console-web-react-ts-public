...
const VocabularyRusPage: React.FC = () => {
  ...
  const [deleteTarget, setDeleteTarget] = React.useState<VocabularyRu | null>(null);
  ...
  const vocabularyRuToFormData = (vocabularyRu: VocabularyRu) => ({
    ...
    lang: vocabularyRu.lang || (dialog.getCurrentLanguage ? dialog.getCurrentLanguage() : 'EN'),
  ...
  const handleVocabularyRuAction = (vocabularyRu: VocabularyRu, action: 'view' | 'edit' | 'delete') => {
  ...
  const handleEditConfirm = async (formData: VocabularyRuFormData) => {
    ...
    dialog.setLoading(true);
    try {
      await vocabularyRuService.updateVocabularyRu(dialog.selectedVocabularyRu.id, {
        word: formData.word,
        definition: formData.definition,
        translation: formData.translation,
        example: formData.example,
