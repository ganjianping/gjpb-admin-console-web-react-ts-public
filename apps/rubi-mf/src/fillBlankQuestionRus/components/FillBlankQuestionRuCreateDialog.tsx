...
interface FillBlankQuestionRuCreateDialogProps {
  open: boolean;
  onClose: () => void;
  onConfirm: (data: FillBlankQuestionRuFormData) => Promise<void>;
}

const FillBlankQuestionRuCreateDialog = ({
  open,
  onClose,
  onConfirm,
}: FillBlankQuestionRuCreateDialogProps) => {
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      console.error("[FillBlankQuestionRuCreateDialog] Error loading tags:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableDifficultyLevels = useMemo(() => {
    try {
      ...
      console.error("[FillBlankQuestionRuCreateDialog] Error loading difficulty levels:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableGrammarChapters = useMemo(() => {
    try {
      ...
      console.error("[FillBlankQuestionRuCreateDialog] Error loading grammar chapters:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableScienceChapters = useMemo(() => {
    try {
      ...
      console.error("[FillBlankQuestionRuCreateDialog] Error loading science chapters:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  useEffect(() => {
    if (open) {
      ...
      setFormData({ ...getEmptyFillBlankQuestionRuFormData(), lang: defaultLang });
      setErrors({});
      setApiErrorMessage("");
    }
  }, [open, i18n.language]);
...
export default FillBlankQuestionRuCreateDialog;
