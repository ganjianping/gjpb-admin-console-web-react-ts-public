...
import type { FreeTextQuestionRuSearchFormData } from '../types/freeTextQuestionRu.types';
import { 
  FREE_TEXT_QUESTION_TAG_SETTING_KEY, 
  LANGUAGE_OPTIONS, 
  DIFFICULTY_LEVEL_OPTIONS,
  FREE_TEXT_QUESTION_LANG_SETTING_KEY,
  FREE_TEXT_QUESTION_DIFFICULTY_LEVEL_SETTING_KEY
} from '../constants';

interface FreeTextQuestionRuSearchPanelProps {
  searchFormData: FreeTextQuestionRuSearchFormData;
  loading?: boolean;
  onFormChange: (field: keyof FreeTextQuestionRuSearchFormData, value: any) => void;
  onSearch: () => void;
  onClear: () => void;
}

const FreeTextQuestionRuSearchPanel: React.FC<FreeTextQuestionRuSearchPanelProps> = ({
  searchFormData,
  loading,
  onFormChange,
  onSearch,
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      const tagSetting = appSettings.find((s) => s.name === FREE_TEXT_QUESTION_TAG_SETTING_KEY && s.lang === currentLang);
      if (!tagSetting) return [] as string[];
      return tagSetting.value.split(',').map((v) => v.trim()).filter(Boolean);
    } catch (err) {
      console.error('[FreeTextQuestionRuSearchPanel] Error loading tags:', err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableLangOptions = useMemo(() => {
    try {
      ...
      const currentLang = i18n.language.toUpperCase().startsWith('ZH') ? 'ZH' : 'EN';
      const langSetting =
        appSettings.find((s) => s.name === FREE_TEXT_QUESTION_LANG_SETTING_KEY && s.lang === currentLang) ||
        appSettings.find((s) => s.name === FREE_TEXT_QUESTION_LANG_SETTING_KEY);
      ...
      console.error('[FreeTextQuestionRuSearchPanel] Error loading lang options:', err);
  ...
  const availableDifficultyOptions = useMemo(() => {
    try {
      ...
      const difficultySetting =
        appSettings.find((s) => s.name === FREE_TEXT_QUESTION_DIFFICULTY_LEVEL_SETTING_KEY && s.lang === currentLang) ||
        appSettings.find((s) => s.name === FREE_TEXT_QUESTION_DIFFICULTY_LEVEL_SETTING_KEY);
      ...
      console.error('[FreeTextQuestionRuSearchPanel] Error loading difficulty options:', err);
  ...
  return (
    ...
    >
      <CardContent sx={{ p: 3 }}>
        ...
        >
          <Search size={20} />
          {t('freeTextQuestionRus.search')}
        </Typography>

        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: 'repeat(4, 1fr)' }, gap: 2, mb: 3 }}>
          <FormControl fullWidth>
            <FormLabel sx={{ mb: 1, fontSize: '0.875rem', fontWeight: 500 }}>
              {t('freeTextQuestionRus.filters.searchByQuestion')}
            </FormLabel>
            <TextField
              size="small"
              placeholder={t('freeTextQuestionRus.filters.searchByQuestion')}
          ...
          <FormControl fullWidth>
            <FormLabel sx={{ mb: 1, fontSize: '0.875rem', fontWeight: 500 }}>
              {t('freeTextQuestionRus.form.lang')}
            </FormLabel>
            <Select
              size="small"
            ...
            >
              <MenuItem value="">{t('freeTextQuestionRus.filters.all')}</MenuItem>
          ...
          <FormControl fullWidth>
            <FormLabel sx={{ mb: 1, fontSize: '0.875rem', fontWeight: 500 }}>
              {t('freeTextQuestionRus.form.tags')}
            </FormLabel>
            <Select
              size="small"
          ...
          <FormControl fullWidth>
            ...
            >
              <MenuItem value="">{t('freeTextQuestionRus.filters.all')}</MenuItem>
              <MenuItem value="true">{t('freeTextQuestionRus.status.active')}</MenuItem>
              <MenuItem value="false">{t('freeTextQuestionRus.status.inactive')}</MenuItem>
            </Select>
          </FormControl>
        </Box>
        ...
        <Box sx={{ display: 'flex', gap: 1.5, justifyContent: 'flex-end' }}>
          ...
          >
            {t('freeTextQuestionRus.clearFilters')}
          </Button>
          <Button
            variant="contained"
...
export default FreeTextQuestionRuSearchPanel;
