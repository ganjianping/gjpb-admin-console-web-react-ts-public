...
const FreeTextQuestionRuViewDialog = ({ open, freeTextQuestionRu, onClose, onEdit }: FreeTextQuestionRuViewDialogProps) => {
  ...
  return (
    ...
    >
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2.5 }}>
                <Box>
                  <Typography variant="caption" sx={{ color: 'text.secondary', mb: 0.5, display: 'block' }}>
                    {t('freeTextQuestionRus.form.correctAnswer')}
                  </Typography>
                ...
                <Box>
                  ...
                  <Chip label={languageLabel(freeTextQuestionRu.lang)} size="small" sx={{ fontWeight: 600 }} />
                </Box>
                <Box>
                  <Typography variant="caption" sx={{ color: 'text.secondary', mb: 0.5, display: 'block' }}>
                    {t('freeTextQuestionRus.form.difficultyLevel')}
                  </Typography>
                  <Typography variant="body2">{freeTextQuestionRu.difficultyLevel || '-'}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" sx={{ color: 'text.secondary', mb: 0.5, display: 'block' }}>
                    {t('freeTextQuestionRus.form.displayOrder')}
                  </Typography>
                  <Typography variant="body2">{freeTextQuestionRu.displayOrder ?? '-'}</Typography>
                </Box>
                <Box sx={{ gridColumn: '1 / -1' }}>
                  <Typography variant="caption" sx={{ color: 'text.secondary', mb: 0.5, display: 'block' }}>
                    {t('freeTextQuestionRus.form.explanation')}
                  </Typography>
                  <Typography variant="body2">{freeTextQuestionRu.explanation || '-'}</Typography>
                </Box>
              </Box>
            </CardContent>
          </Card>
          ...
          <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider' }}>
            <CardContent sx={{ p: 3 }}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2.5 }}>
                <Box>
                  <Typography variant="caption" sx={{ color: 'text.secondary', mb: 0.5, display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    <Calendar size={12} /> {t('freeTextQuestionRus.form.createdAt')}
                  </Typography>
                  <Typography variant="body2">
                    {freeTextQuestionRu.createdAt ? format(parseISO(freeTextQuestionRu.createdAt), 'yyyy-MM-dd HH:mm') : '-'}
                  </Typography>
                </Box>
                <Box>
                  <Typography variant="caption" sx={{ color: 'text.secondary', mb: 0.5, display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    <Calendar size={12} /> {t('freeTextQuestionRus.form.updatedAt')}
                  </Typography>
                  <Typography variant="body2">
                    {freeTextQuestionRu.updatedAt ? format(parseISO(freeTextQuestionRu.updatedAt), 'yyyy-MM-dd HH:mm') : '-'}
                  </Typography>
                </Box>
                <Box>
                  <Typography variant="caption" sx={{ color: 'text.secondary', mb: 0.5, display: 'block' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    {freeTextQuestionRu.isActive ? (
                      <Chip icon={<CheckCircle2 size={14} />} label={t('freeTextQuestionRus.status.active')} size="small" color="success" sx={{ fontWeight: 500 }} />
                    ) : (
                      <Chip icon={<XCircle size={14} />} label={t('freeTextQuestionRus.status.inactive')} size="small" color="default" sx={{ fontWeight: 500 }} />
                    )}
                  </Box>
                </Box>
                <Box>
      ...
      <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
        ...
        {onEdit && (
          <Button onClick={() => onEdit(freeTextQuestionRu)} variant="contained">
            {t('common.edit')}
          </Button>
        )}
      </DialogActions>
