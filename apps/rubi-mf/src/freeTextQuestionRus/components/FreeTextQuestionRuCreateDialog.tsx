...
interface FreeTextQuestionRuCreateDialogProps {
  open: boolean;
  onClose: () => void;
  onConfirm: (data: FreeTextQuestionRuFormData) => Promise<void>;
}

const FreeTextQuestionRuCreateDialog = ({
  open,
  onClose,
  onConfirm,
}: FreeTextQuestionRuCreateDialogProps) => {
  ...
  const availableTags = useMemo(() => {
    try {
      ...
      console.error("[FreeTextQuestionRuCreateDialog] Error loading tags:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  const availableDifficultyLevels = useMemo(() => {
    try {
      ...
      console.error("[FreeTextQuestionRuCreateDialog] Error loading difficulty levels:", err);
      return [] as string[];
    }
  }, [i18n.language]);
  ...
  useEffect(() => {
    if (open) {
      ...
      setFormData({ ...getEmptyFreeTextQuestionRuFormData(), lang: defaultLang });
      setErrors({});
      setApiErrorMessage("");
    }
  }, [open, i18n.language]);
...
export default FreeTextQuestionRuCreateDialog;
